...
void event_selector(int N_Events, vector<myParticle> **Particles, vector<int> &selected_events, bool p_type);
...
int main() {
    ...
    for (int i = 0; i < input.N_simulations; ++i) {
        ...
        event_selector(input.N_events[i], Particles, selected_events, true);
        write_txt(target_np_dir + input.files[i], selected_events, Particles, true);
        event_selector(input.N_events[i], Particles, selected_events, false);
        write_txt(target_npbar_dir + input.files[i], selected_events, Particles, false);
    ...
    for (int i = 0; i < N_Events; ++i) {
        ...
        for (auto &proton:Particles[i][p_types[0]]) {
            for (auto &neutron:Particles[i][p_types[1]]) {
                ...
                if (abs(deuteron.y()) <= 0.5) { usefull_event = true; }
            }
        }
